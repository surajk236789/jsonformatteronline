...
export default function JsonSchemaValidator() {
  ...
  return (
    <div className="bg-panel rounded-2xl shadow-sm border border-panel-border p-6">
      ...
      {result && (
        <div
          className={`mt-4 p-4 rounded-xl text-sm whitespace-pre-wrap ${result.valid ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"
            }`}
        >
          <div className="font-bold mb-1">
            {result.valid ? "✅ Success" : "❌ Validation Errors"}
          </div>
          {result.message}
        </div>
      )}
