...
const SAMPLE_TEXT = `https://example.com/search?q=hello world & welcome!`;
    ...
    <div className="w-full p-2 md:p-4 lg:p-6 rounded-2xl glass-panel shadow-lg border border-slate-200/40 dark:border-slate-800/40 transition-all duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col">
          ...
          <textarea
            className="w-full h-80 md:h-[420px] p-4 font-mono text-sm border border-panel-border rounded-xl bg-panel focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:border-transparent transition-all duration-200 shadow-inner resize-none"
        ...
        <div className="flex flex-col">
          ...
          <div className="relative w-full h-80 md:h-[420px] border border-panel-border rounded-xl bg-background overflow-hidden shadow-inner flex flex-col">
            {output ? (
              <pre className="w-full flex-1 p-4 font-mono text-sm overflow-auto text-cyan-600 dark:text-cyan-400 select-all whitespace-pre-wrap break-all">
                <code>{output}</code>
              </pre>
            ) : (
            ...
            {output && (
              <div className="absolute top-3 right-3 flex items-center gap-2">
                ...
                <Button
                  ...
                  className="p-2 bg-cyan-600/90 hover:bg-cyan-600 text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer shadow-md"
                >
                  {downloadSuccess ? (
                    <>
                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="flex flex-wrap items-center justify-end gap-3 mt-6 pt-6 border-t border-panel-border/50">
        ...
        <Button
          onClick={decode}
          variant="secondary"
          className="border-cyan-200 hover:border-cyan-300 dark:border-cyan-800 dark:hover:border-cyan-700 text-cyan-700 dark:text-cyan-300 bg-cyan-50/50 hover:bg-cyan-100/50 dark:bg-cyan-900/20 dark:hover:bg-cyan-900/40"
        >
        ...
        <Button
          onClick={encode}
          className="glow-button px-6 py-2.5 bg-cyan-600 hover:bg-cyan-700 text-white text-sm font-bold rounded-lg transition shadow-md cursor-pointer flex items-center gap-2"
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
