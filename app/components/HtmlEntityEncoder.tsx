...
const SAMPLE_TEXT = `<div class="welcome">
  <h1>Hello & Welcome!</h1>
  <p>This is a "sample" string showing <various> entities & symbols like © and ™.</p>
</div>`;
...
export default function HtmlEntityEncoder() {
  ...
  const downloadTxt = useCallback(() => {
    ...
    a.href = url;
    a.download = "encoded-entities.txt";
    a.click();
    ...
    <div className="w-full p-2 md:p-4 lg:p-6 rounded-2xl glass-panel shadow-lg border border-slate-200/40 dark:border-slate-800/40 transition-all duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col">
          ...
          <div className="relative w-full h-80 md:h-[420px] border border-panel-border rounded-xl bg-background overflow-hidden shadow-inner flex flex-col">
            {output ? (
              <pre className="w-full flex-1 p-4 font-mono text-sm overflow-auto text-teal-600 dark:text-teal-400 select-all whitespace-pre-wrap break-all">
                <code>{output}</code>
              </pre>
            ) : (
      ...
      <div className="flex flex-wrap items-center justify-end gap-3 mt-6 pt-6 border-t border-panel-border/50">
        ...
        <Button
          onClick={decode}
          variant="secondary"
          className="border-teal-200 hover:border-teal-300 dark:border-teal-800 dark:hover:border-teal-700 text-teal-700 dark:text-teal-300 bg-teal-50/50 hover:bg-teal-100/50 dark:bg-teal-900/20 dark:hover:bg-teal-900/40"
        >
