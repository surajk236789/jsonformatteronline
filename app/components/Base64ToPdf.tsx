...
function DropZone({ onFile }: { onFile: (content: string, name: string) => void }) {
  ...
  return (
    <div
      ...
      className={`cursor-pointer flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed py-5 px-4 text-center transition-all duration-200 select-none ${
        dragging
          ...
          : "border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 hover:border-indigo-400 hover:bg-indigo-50/40 dark:hover:bg-indigo-900/10"
      }`}
      role="button"
    ...
    >
      ...
      />
      <svg
        className={`w-8 h-8 transition-colors ${dragging ? "text-indigo-500" : "text-slate-400 dark:text-slate-600"}`}
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
      >
      ...
      <p className="text-[10px] text-slate-400 dark:text-slate-600">Supports multiple files for batch conversion</p>
    </div>
  );
}
    ...
    <div className="flex flex-col gap-2 h-full">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <button
          ...
          title="Zoom out"
          aria-label="Zoom out"
          className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 transition disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
        >
        ...
        <span className="text-xs font-mono text-slate-600 dark:text-slate-300 min-w-[3rem] text-center">{zoom}%</span>
        <button
      ...
      <div className="relative flex-1 overflow-auto rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950 shadow-inner min-h-[300px]">
        <div
...
function BatchItemRow({ item, onDownload, onRemove }: { item: BatchItem; onDownload: (item: BatchItem) => void; onRemove: (id: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-sm transition-all hover:shadow-sm">
      <span className={`w-2 h-2 rounded-full shrink-0 ${statusColor[item.status]}`} />
      <span className="flex-1 font-mono text-xs text-slate-700 dark:text-slate-300 truncate min-w-0" title={item.name}>{item.name}</span>
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4 md:p-6 rounded-2xl glass-panel shadow-lg border border-slate-200/40 dark:border-slate-800/40 transition-all duration-300">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        <div className="inline-flex rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-900 p-1 gap-1">
          <button
            onClick={() => setBatchMode(false)}
            id="mode-single"
            className={`px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${!batchMode ? "bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow" : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"}`}
          >
            Single
          </button>
          <button
        ...
        {!batchMode && (
          <button onClick={loadSample} id="load-sample-btn" className="px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold rounded-lg transition cursor-pointer">
            📄 Load Sample
          </button>
        )}
        {batchMode && batchItems.length > 0 && (
          <>
            <button onClick={clearBatch} id="clear-batch-btn" className="px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-semibold rounded-lg transition cursor-pointer">
              🗑 Clear All
            </button>
      ...
      {!batchMode && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <textarea
                ...
                className="w-full h-72 md:h-[380px] p-4 font-mono text-xs sm:text-sm border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition-all duration-200 shadow-inner resize-y text-slate-800 dark:text-slate-200 placeholder:text-slate-400"
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="w-full h-72 md:h-[380px] border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50 dark:bg-slate-950 overflow-hidden shadow-inner flex flex-col">
                {pdfUrl && previewReady ? (
                  <div className="p-2 flex-1 flex flex-col overflow-hidden">
                    <PdfPreview pdfUrl={pdfUrl} />
                  </div>
                ) : (
                  <div className="flex-1 flex flex-col justify-center items-center p-8 text-center text-slate-400 dark:text-slate-600">
                    {converting ? (
                      <>
          ...
          {/* Action Buttons */}
          <div className="flex flex-wrap items-center justify-end gap-3 mt-5 pt-5 border-t border-slate-200 dark:border-slate-700/50">
            <button onClick={handleClear} id="clear-btn" className="px-5 py-2.5 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 text-sm font-semibold rounded-lg transition cursor-pointer flex items-center gap-2">
        ...
        <div className="flex flex-col gap-4">
          ...
          {batchItems.length > 0 ? (
            <>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-slate-200 dark:border-slate-700/50">
                <span className="text-xs text-slate-500 dark:text-slate-400">
                  {batchItems.filter((i) => i.status === "done").length} / {batchItems.length} converted
                </span>
                <button onClick={convertAllBatch} id="batch-convert-btn" className="glow-button px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-lg transition shadow-md cursor-pointer flex items-center gap-2">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
                  </svg>
                  Convert All
            ...
            <div className="flex flex-col items-center justify-center gap-3 py-12 text-slate-400 dark:text-slate-600">
      ...
      <div className="mt-6 pt-5 border-t border-slate-200/60 dark:border-slate-700/40">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            ...
            <div key={card.title} className="flex items-start gap-2.5 p-3 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800/50">
              <span className="text-lg mt-0.5">{card.icon}</span>
              <div>
                <p className="text-xs font-bold text-slate-700 dark:text-slate-200">{card.title}</p>
