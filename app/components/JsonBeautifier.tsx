...
export default function JsonBeautifier() {
  ...
  const handleFormat = () => {
    ...
    try {
      ...
      setError(errMsg || "Invalid JSON input");
      setOutput("");
    }
  };
    ...
    <div className="w-full p-2 md:p-4 lg:p-6 rounded-2xl glass-panel shadow-lg border border-slate-200/40 dark:border-slate-800/40 transition-all duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col">
          ...
          <textarea
            className="w-full h-80 md:h-[400px] p-4 font-mono text-sm border border-panel-border rounded-xl bg-panel focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition-all duration-200 shadow-inner"
        ...
        <div className="flex flex-col">
          <label className="text-xs font-bold text-secondary uppercase tracking-wider mb-2 flex items-center justify-between">
            <span>{"Beautified / Minified Output"}</span>
            {output && (
            ...
            {output && (
              <div className="absolute top-3 right-3 flex items-center gap-2">
                ...
                <Button
                  ...
                  title="Download JSON"
                  className="p-2 bg-emerald-600/90 hover:bg-emerald-600 text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer shadow-md"
                >
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
