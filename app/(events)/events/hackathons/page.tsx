...
export const metadata: Metadata = {
  ...
  description: 'Find the latest and greatest upcoming hackathons around the world.',
};
...
export default async function HackathonsPage() {
  ...
  const events = allEvents.filter(e => e.tags.some(t => t.toLowerCase() === 'hackathon'));
      ...
      <div className="max-w-7xl mx-auto space-y-16">
        ...
        <div className="text-center space-y-6 relative">
          <div className="absolute inset-0 flex items-center justify-center -z-10 opacity-20 pointer-events-none">
            <div className="w-[600px] h-[600px] bg-green-600 rounded-full blur-[120px] mix-blend-screen"></div>
          </div>
          <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-emerald-400 drop-shadow-sm">
            Upcoming Hackathons
          </h1>
          <p className="text-xl md:text-2xl text-gray-400 max-w-3xl mx-auto leading-relaxed">
            Build, ship, and win. Discover hackathons to test your skills and build the next big thing.
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-4">
          ...
          <Link href="/events/hackathons" className="px-6 py-2.5 rounded-full bg-white/10 hover:bg-white/20 border border-white/5 backdrop-blur-md transition-all font-medium">
            Hackathons
          </Link>
        ...
        {events.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {events.map((event) => (
              <Link key={event.slug} href={`/events/${event.slug}`} className="group relative block h-full">
                <div className="absolute inset-0 bg-gradient-to-b from-green-500/20 to-emerald-500/20 rounded-3xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                <div className="relative h-full flex flex-col bg-white/[0.03] hover:bg-white/[0.06] border border-white/10 rounded-3xl p-8 backdrop-blur-xl transition-all duration-300 group-hover:-translate-y-1">
                  ...
                  <div className="flex gap-2 mb-6 flex-wrap">
                    {event.tags.slice(0, 3).map((tag, idx) => (
                      <span key={idx} className="px-3 py-1 text-xs font-semibold uppercase tracking-wider bg-green-500/20 text-green-300 rounded-full border border-green-500/30">
                        {tag}
                      </span>
                    ))}
                  </div>

                  <h3 className="text-2xl font-bold mb-4 text-gray-100 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-green-400 group-hover:to-emerald-400 transition-all">
                    {event.title}
                  </h3>
          ...
          <div className="text-center py-20 bg-white/5 rounded-3xl border border-white/10 backdrop-blur-md">
            <h3 className="text-2xl font-semibold text-gray-300 mb-2">No hackathons found</h3>
